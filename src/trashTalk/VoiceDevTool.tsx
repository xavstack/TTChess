...
import { listVoices, setPreferredVoice, setVoiceRatePitch, setPreferredAudioUrl, cancelSpeech, speak } from './tts';
  ...
  return (
    <div className="mt-4 border rounded-md p-3">
      <div className="flex items-center justify-between">
        ...
        <button className="text-xs rounded border px-2 py-1" onClick={() => setOpen(!open)}>{open ? 'Hide' : 'Show'}</button>
      </div>
      {open && (
        <div className="mt-3 space-y-2">
          <div>
            ...
            >
              ...
              {voices.map((v) => (
                <option key={v.uri} value={v.uri}>{v.name} {v.lang ? `(${v.lang})` : ''}</option>
              ))}
            </select>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              ...
              <input type="range" min={0} max={2} step={0.01} value={pitch}
                onChange={(e) => setPitch(Number(e.target.value))} className="w-full" />
            </div>
          </div>

          <div>
            <label className="block text-xs mb-1">Override with Audio URL (for custom voices; MP3/WAV/OGG)</label>
            <input className="w-full rounded border p-1.5 text-xs bg-white dark:bg-black"
