...
let preferredVoiceId: string | null = ((): string | null => localStorage.getItem('ttc_voice_id_v1'))();
...
function playNext(): void {
  ...
  if (!next) { isSpeaking = false; return; }
  isSpeaking = true;
  if (next.kind === 'audio') {
    const a = next.audio;
    a.onended = () => { isSpeaking = false; lastSpokenAt = Date.now(); playNext(); };
    a.onerror = () => { isSpeaking = false; playNext(); };
    try { a.currentTime = 0; a.play().catch(() => { isSpeaking = false; }); } catch { isSpeaking = false; }
    return;
  }
  if (!s) { isSpeaking = false; queue.length = 0; return; }
  const u = next.utter;
  u.onend = () => { isSpeaking = false; lastSpokenAt = Date.now(); playNext(); };
  u.onerror = () => { isSpeaking = false; playNext(); };
  s.speak(u);
}
...
export function cancelSpeech(): void {
  ...
  for (const item of queue) {
    if (item.kind === 'audio') try { item.audio.pause(); } catch {}
  }
  queue.length = 0;
  isSpeaking = false;
...
(function ensureVoicesReady() {
  ...
  if (!s.getVoices().length) {
    s.onvoiceschanged = () => { /* populated lazily */ };
  }
})();
