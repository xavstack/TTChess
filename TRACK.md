...
- stockfish-refactor: Added optional Stockfish WASM worker with auto-fallback to heuristic engine; strengthened heuristic AI with depth-limited negamax search; fixed avatar display with fallback image; defaulted to Merida piece set; enabled PNG piece assets; fixed all lint/TypeScript errors; build verified complete with all assets.
